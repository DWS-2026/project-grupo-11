...
public class RatingRestController {
    ...
    public ResponseEntity<Page<RatingDTO>> getMyRatings(
        ...
        Page<Rating> ratings = ratingService.findByAuthorUsername(userDetails.getUsername(), pageable);
        Page<RatingDTO> dtos = ratings.map(ratingMapper::toDTO);
        return ResponseEntity.ok(dtos);
    }

    // GET /api/v1/ratings/my-ratings/{id}
    @GetMapping("/ratings/my-ratings/{id}")
    public ResponseEntity<RatingDTO> getMyRatingById(@PathVariable Long id, @AuthenticationPrincipal UserDetails userDetails) {
        if (userDetails == null) {
            return ResponseEntity.status(401).build();
        }
        Optional<Rating> ratingOp = ratingService.findById(id);
        ...
        if(!rating.getAuthor().getUsername().equalsIgnoreCase(userDetails.getUsername())){
            return ResponseEntity.status(403).build();
        }
